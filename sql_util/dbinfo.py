...
# Initialize the T5 model and tokenizer globally to avoid reloading
tokenizer = T5Tokenizer.from_pretrained('t5-small')
model = T5Model.from_pretrained('t5-small')

def generate_schema_embeddings(table_name: str, column_names: list) -> torch.Tensor:
    """
    Generate embeddings for table name and columns using T5.
    """
    input_text = f"Table: {table_name}, Columns: {', '.join(column_names)}"
    inputs = tokenizer(input_text, return_tensors="pt")
    outputs = model(**inputs)
    return outputs.last_hidden_state.mean(dim=1)  # Return mean pooling of embeddings


# Update extract_table_column_properties_path to use embeddings
def extract_table_column_properties_path(sqlite_path: str) -> Tuple[Dict[str, Dict[str, Any]], Dict[Tuple[str, str], Tuple[str, str]]]:
    ...
    for table_name in table_names:
        ...
        # Get column details and generate embeddings
        result_type, result = exec_db_path_(sqlite_path, column_type_query % table_name, nocase=False)
        column_names = []
        for columnID, column_name, columnType, columnNotNull, columnDefault, columnPK in result:
        ...
        embeddings = generate_schema_embeddings(table_name, column_names)
        table_name2column_properties[table_name.lower()]['embeddings'] = embeddings

        # Extract foreign keys and populate child2parent
        result_type, result = exec_db_path_(sqlite_path, foreign_key_query % table_name, nocase=False)
        for keyid, column_seq_id, other_tab_name, this_column_name, other_column_name, *_ in result:
